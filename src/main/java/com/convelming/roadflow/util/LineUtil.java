...
public class LineUtil {
    ...
    public static boolean crossJudgment(double[][] vertex, Coord[] line) {
        List<LineSegment> lineSegments1 = new ArrayList<>(), lineSegments2 = new ArrayList<>();
        for (int i = 0; i < vertex.length - 1; i++) {
            lineSegments1.add(new LineSegment(new Point(vertex[i][0], vertex[i][1]), new Point(vertex[i + 1][0], vertex[i + 1][1])));
        }
        lineSegments2.add(new LineSegment(new Point(line[0].getX(), line[0].getY()), new Point(line[1].getX(), line[1].getY())));
